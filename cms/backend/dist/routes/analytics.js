...
function hashIp(ip) {
    return (0, crypto_1.createHash)('sha256').update(ip + (process.env.IP_HASH_SALT || 'nua')).digest('hex').slice(0, 16);
}
const pageviewSchema = zod_1.z.object({
    page: zod_1.z.string().min(1).max(200),
    referrer: zod_1.z.string().max(500).optional(),
    sessionId: zod_1.z.string().max(64).optional(),
});
const eventSchema = zod_1.z.object({
    type: zod_1.z.string().min(1).max(60),
    page: zod_1.z.string().min(1).max(200),
...
async function analyticsRoutes(app) {
    ...
    app.get('/dashboard', {
        ...
        const [totalVisitors, pageViewsRaw, topPages, topReferrers, eventCounts, demoBookings, bookingsBySource,] = await Promise.all([
            // Total unique visitors (by ipHash) in period
            db_1.prisma.pageView.groupBy({
                by: ['ipHash'],
            ...
            db_1.prisma.pageView.count({ where: { createdAt: { gte: since } } }),
            ...
            db_1.prisma.demoBooking.findMany({
                where: { createdAt: { gte: since } },
            ...
            db_1.prisma.demoBooking.groupBy({
                by: ['source'],
                where: { createdAt: { gte: since } },
                _count: { id: true },
                orderBy: { _count: { id: 'desc' } },
        ...
        let dailyVisitors = [];
        try {
            dailyVisitors = await db_1.prisma.$queryRaw `
        SELECT
