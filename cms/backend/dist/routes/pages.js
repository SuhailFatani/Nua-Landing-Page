...
const VALID_SLUGS = ['home', 'pricing', 'services', 'company', 'blog', 'book_a_demo'];
const updatePageSchema = zod_1.z.object({
    title: zod_1.z.string().min(1).max(160).optional(),
    metaDesc: zod_1.z.string().max(320).optional(),
    content: zod_1.z.record(zod_1.z.unknown()), // flexible JSON — serialized to string in SQLite
...
async function pagesRoutes(app) {
    ...
    app.get('/:slug', async (request, reply) => {
        ...
        const page = await db_1.prisma.page.findUnique({ where: { slug } });
    ...
    app.put('/:slug', {
        ...
        const oldPage = await db_1.prisma.page.findUnique({ where: { slug } });
        const updated = await db_1.prisma.page.upsert({
            where: { slug },
            update: {
